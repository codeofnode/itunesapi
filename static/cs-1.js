...
GLOBAL_METHODS.store = (function(require, GLOBAL_APP_CONFIG, GLOBAL_METHODS) {
...
var GLOBAL_API = {
  "root": {
    ...
    ":search": {
      "$": {
        "from": "/api/search?q={{params.path.search}}",
        "@": "showAlbum"
      }
    }
  }
...
GLOBAL_API.root._methods.onReady = (function(GLOBAL_APP_CONFIG, GLOBAL_METHODS, GLOBAL_VARS, GLOBAL_API) {
  ...
  function func(vars, m, req) {
    setTimeout(function() {
      var searchEl = document.querySelector('#home-' + GLOBAL_VARS.app.key + '-search');
      if (searchEl && !done[0]) {
        done[0] = true;
        searchEl.addEventListener('keypress', function(e) {
          var key = e.which || e.keyCode;
          if (key === 13) {
            r2('/' + e.target.value.replace(/\ /g, '+'));
          }
        });
      }
      var topEl = document.querySelector('#top-fixed-search');
      if (topEl && !done[1]) {
        done[1] = true;
        topEl.addEventListener('keypress', function(e) {
          (document.querySelectorAll('.mdl-cell.mdl-card') || []).forEach(function(ab) {
            ab.style.display = (e.target.value && ab.querySelector('h4').innerText.toLowerCase().indexOf(e.target.value.toLowerCase()) === -1) ? 'none' : 'block';
          });
        });
      }
    }, 300);
...
GLOBAL_API.root._methods.renderHomePage = (function(GLOBAL_APP_CONFIG, GLOBAL_METHODS, GLOBAL_VARS, GLOBAL_API) {
  ...
  var HTML = ('<center>  <h4>search below</h4>  <div class="mdl-textfield mdl-js-textfield">    <input class="mdl-textfield__input" type="text" id="home-cs-search">    <label class="mdl-textfield__label">enter artists names</label>  </div></center>');
  var done = false;

  function func(vars, methods, req, res, next) {
    document.getElementById('search-top-ele-id').style.display = 'none';
    if (done) {
      document.getElementById('current-' + vars.app.key + '-title').innerHTML = vars.app.title;
    } else {
      done = true;
      methods.onReady(vars, methods, req);
      next(HTML);
    }
  }
...
GLOBAL_METHODS.makeToLast(GLOBAL_API.root, '_methods', 'showAlbum');
GLOBAL_API.root._methods.showAlbum = (function(GLOBAL_APP_CONFIG, GLOBAL_METHODS, GLOBAL_VARS, GLOBAL_API) {

  var HTML = ('<div class="mdl-cell mdl-cell--3-col mdl-cell--4-col-tablet mdl-cell--4-col-phone mdl-card mdl-shadow--3dp">  <div class="mdl-card__media">    <img src="{{imageUrl}}">  </div>  <div class="mdl-card__title">    <h4 class="mdl-card__title-text">{{title}}</h4>  </div>  <div class="mdl-card__supporting-text">    <span class="mdl-typography--font-light mdl-typography--subhead">{{description}}</span>  </div>  <div class="mdl-card__actions">    <a class="android-link mdl-button mdl-js-button mdl-typography--text-uppercase" target="_blank" href="{{viewUrl}}" data-upgraded=",MaterialButton">       Explore More       <i class="material-icons">chevron_right</i>     </a>  </div></div>');

  function addOne(data) {
    if (!data) return '';
    var img = (data.artworkUrl100 && data.artworkUrl100.split('/'));
    if (img) {
      img.pop();
      img.push('600x400bb.jpg');
      img = img.join('/');
    } else {
      img = 'http://via.placeholder.com/350x150';
    }
    return GLOBAL_METHODS.replace(HTML, {
      title: (data.collectionName && data.collectionName.substring(0, 20)) || 'no title',
      description: (data.artistName && data.artistName.substring(0, 40)) || '',
      imageUrl: img,
      viewUrl: data.collectionViewUrl,
    });
  }

  function func(vars, methods, req, res, next) {
    document.getElementById('current-' + vars.app.key + '-title').innerHTML = vars.params.path.search.replace('+', ' ');
    document.getElementById('search-top-ele-id').style.display = 'block';
    var ar = vars.currentData,
      ln = ar.length;
    var htmls = '';
    for (var z = 0; z < ln; z += 4) {
      htmls += '<div class="android-card-container mdl-grid">';
      htmls += (addOne(ar[z]) + addOne(ar[z + 1]) + addOne(ar[z + 2]) + addOne(ar[z + 3]));
      htmls += '</div>';
    }
    next(htmls ? htmls : '<h1>No albums found. Go <a style="cursor:pointer" onclick="r2(\'/\')">back</a> and retry.</h1>');
  }
